...
void FFTModule::signalData(float** data, uint samplesNumber) {
    double** output = new double*[samplesNumber];

    for (uint i = 0; i < samplesNumber; i++) {
        for (uint j = 0; j < fftSize; j++) array[j] = data[i][j];
        ...
        uint j;
        output[i] = new double[((fftSize / 2) + 1)];
    ...
    emit calculated(output, samplesNumber);
